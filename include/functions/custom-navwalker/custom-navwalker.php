...
class custom_navwalker extends Walker_Nav_Menu{
	...
	public function start_el( &$output, $item, $depth = 0, $args = [], $id = 0 ) {
		...
		if(isset($item->hide_title) && $item->hide_title == 1){
			$hide_title = true;
			$link_html = '<i class="kgm-hide">' . $link_html . '</i>';
		}else{
			$hide_title = false;
		}
...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->_invalid ) ) {
			...
			/* translators: %s: tit le of menu item which is invalid */
			$title = sprintf( '%s (无效)' , $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( '%s (待定)', $item->title );
		}
		...
		<li id="menu-item-<?= $item_id; ?>" class="<?= implode(' ', $classes ); ?>">
			<dl class="menu-item-bar">
				<dt class="menu-item-handle">
					...
					<span class="item-controls">
						...
						<a class="item-edit" id="edit-<?= $item_id; ?>" title="编辑菜单项目" href="<?php
			...
			<div class="menu-item-settings" id="menu-item-settings-<?= $item_id; ?>">
				...
				<!-- awesome icon -->
				<p class="field-awesome description description-wide">
					<label for="edit-menu-item-awesome-<?= $item_id; ?>">
						FontAwesome图标(例如home) <a href="http://www.fontawesome.com.cn/faicons/" target="_blank" title="图标库">图标库</a>
						<br />
						<input type="text" id="edit-menu-item-awesome-<?= $item_id; ?>" class="widefat edit-menu-item-awesome" name="menu-item-awesome[<?= $item_id; ?>]" value="<?= $item->awesome;?>">
					</label>
				</p>

				<!-- only show icon -->
				<p class="field-hide-title description description-wide field-hide-title">
					<label for="edit-menu-item-hide-title-<?= $item_id; ?>">
						...
						<select id="edit-menu-item-hide-title-<?= $item_id; ?>" class="widefat edit-menu-item-hide-title" name="menu-item-hide-title[<?= $item_id; ?>]" >
							<?php if(isset( $item->hide_title ) && $item->hide_title == 1):?>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf( '源站链接: %s' , '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						</p>
					<?php endif; ?>
